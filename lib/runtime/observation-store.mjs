...
export function persistObservation(value, { dataDir } = {}) {
  ...
  return serialized(async () => {
    ...
    const terminalSeen = new Set(
      state.events
        .filter((item) => item.type === "session.terminal")
        .map((item) => `${item.payload.providerSessionId}:${item.payload.status}`),
    );
    const additions = [];
    ...
    for (const session of observation.sessions) {
      ...
      const terminalKey = `${session.providerSessionId}:${session.status}`;
      if (TERMINAL_STATES.has(session.status) && !terminalSeen.has(terminalKey)) {
