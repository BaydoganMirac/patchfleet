...
function validateEvent(value) {
  ...
  if (
    ...
    (!EVENT_TYPES.has(value.type) && !WORK_EVENT_TYPES.has(value.type))
  ) {
    throw new TypeError();
  }
...
async function appendInternal(directory, additions, state) {
  ...
  return { observationProjection, workProjection };
}
...
export async function readWorkProjection({ dataDir } = {}) {
  const directory = dataDirectory(dataDir);
  try {
    const state = await readLog(directory);
    projectWorkEvents(state.events);
    const content = await readFile(join(directory, WORK_PROJECTION_FILE), "utf8");
    return validateWorkProjection(JSON.parse(content));
  } catch (error) {
    if (error.code === "ENOENT") return null;
    if (error instanceof StorageCorruptionError) throw error;
    throw new StorageCorruptionError();
