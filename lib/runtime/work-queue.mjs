...
export function applyWorkCommand(value, { dataDir, now = () => new Date().toISOString() } = {}) {
  ...
  return commitEventTransaction((events) => {
    ...
    if (!existing.requested && intent.expiresAt <= completedAt) {
      outcome = "expired";
      ...
      const alreadyApplied = events.some(
        (candidate) => candidate.type === "work.enqueued" && candidate.payload.intentId === intent.intentId,
      );
      if (alreadyApplied) {
        reasonCode = "WORK_ENQUEUED";
      } else if (projection.items.some((candidate) => candidate.workItemId === item.workItemId)) {
        outcome = "rejected";
      ...
      const alreadyApplied = events.some(
        (candidate) => candidate.type === "work.removed" && candidate.payload.intentId === intent.intentId,
      );
