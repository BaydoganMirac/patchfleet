...
async function requestJson(fetchImpl, url, { method, credential, body }) {
  ...
  if (!response.ok) {
    ...
    throw Object.assign(new Error("Patchfleet Cloud rejected the request"), { code });
  }
...
export function syncCloud({
  ...
  return serialized(async () => {
    ...
    try {
      ...
      const revision = snapshot.events.length;
      const occurredAt = snapshot.events.at(-1)?.recordedAt ?? "1970-01-01T00:00:00.000Z";
      const payload = buildCloudProjection(snapshot.observation, snapshot.work, revision);
      ...
      validateAcceptedResponse(await requestJson(fetchImpl, `${connection.cloudUrl}/api/v1/hosts/${encodeURIComponent(connection.hostId)}/heartbeat`, {
        ...
        body: envelope(connection, `heartbeat:${randomUUID()}`, attemptedAt, {
          appVersion: version,
          protocolVersion: 1,
          lastLocalSequence: revision,
          health: "ok",
        }),
      }));
      validateAcceptedResponse(await requestJson(fetchImpl, `${connection.cloudUrl}/api/v1/hosts/${encodeURIComponent(connection.hostId)}/projection`, {
        method: "PUT",
        credential: connection.credential,
        body: envelope(
          connection,
      ...
      const lastErrorCode = typeof error?.code === "string" && /^[A-Z][A-Z0-9_]{0,63}$/.test(error.code)
        ? error.code
        : "CLOUD_SYNC_FAILED";
