...
import { projectAgentTeamEvents } from "../domain/agent-team.mjs";
import { projectAgentPackEvents } from "../domain/agent-pack.mjs";
import { projectWorkspaceEvents } from "../domain/workspace.mjs";
import { BUILT_IN_AGENT_PACKS } from "../agents/built-in-packs.mjs";
...
import { applyRemoteTeamIntent } from "./team-intents.mjs";
...
async function localSnapshot(dataDir) {
  ...
  return {
    ...
    teams: projectAgentTeamEvents(events),
    workspaces: projectWorkspaceEvents(events),
    agentPacks: [...BUILT_IN_AGENT_PACKS, ...(projectAgentPackEvents(events)?.items.map((item) => item.pack) ?? [])],
  };
}
...
export function pairCloud({ cloudUrl, pairingCode, displayName }, {
  ...
  return serialized(async () => {
    ...
    const consume = async (protocolVersion) => requestJson(fetchImpl, `${origin}/api/v1/pairings/consume`, {
      method: "POST",
      body: {
        schemaVersion: 1,
    ...
    let response;
    try {
      response = validatePairingResponse(await consume(PROTOCOL_VERSION));
    } catch (error) {
      if (error?.code !== "CLOUD_REQUEST_REJECTED") throw error;
      response = validatePairingResponse(await consume(1));
    }
...
export function syncCloud({
  ...
  return serialized(async () => {
    ...
    try {
      ...
      let payload = protocolVersion === 2
        ? buildCloudProjectionV2({
            observation: snapshot.observation,
            work: snapshot.work,
            teams: snapshot.teams,
            workspaces: snapshot.workspaces,
            agentPacks: snapshot.agentPacks ?? BUILT_IN_AGENT_PACKS,
            revision,
            installationId: state.installationId,
          })
        : buildCloudProjection(snapshot.observation, snapshot.work, revision);
      const version = await appVersion();
      const api = `${connection.cloudUrl}/api/v${protocolVersion}/hosts/${encodeURIComponent(connection.hostId)}`;

      validateAcceptedResponse(await requestJson(fetchImpl, `${api}/heartbeat`, {
        method: "POST",
      ...
      const sendProjection = () => requestJson(fetchImpl, `${api}/projection`, {
        method: "PUT",
        credential: connection.credential,
        body: envelope(
          connection,
      ...
      const intentResponse = await requestJson(
        fetchImpl,
        `${api}/intents?after=${connection.cursor}`,
        { method: "GET", credential: connection.credential },
      );
      const page = protocolVersion === 2
        ? validateIntentPageV2(intentResponse, connection.hostId, connection.cursor, attemptedAt)
        : validateIntentPage(intentResponse, connection.hostId, connection.cursor, attemptedAt);
      ...
      for (const { sequence, intent } of page.intents) {
        const receipt = intent.schemaVersion === 2
          ? await applyRemoteTeamIntent(intent, { dataDir, installationId: state.installationId, teamOptions: { providerAvailable } })
          : await applyIntent(intent, { dataDir, providerAvailable });
        const receiptBody = envelope(
          connection,
          stableMessageId("receipt", receipt.intentId),
          receipt.completedAt,
          protocolVersion === 2 ? buildRemoteReceiptPayload(receipt) : buildReceiptPayload(receipt),
        );
        validateAcceptedResponse(await requestJson(fetchImpl, `${api}/receipts`, {
          method: "POST",
          credential: connection.credential,
          body: receiptBody,
        }));
