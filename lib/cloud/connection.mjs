...
function exactWithOptional(value, fields, optional, name) {
  if (!value || typeof value !== "object" || Array.isArray(value)) throw new TypeError(`${name} is invalid`);
  const keys = Object.keys(value);
  if (
...
export function validateCloudState(value) {
  ...
  exactWithOptional(connection, [
    "cloudUrl", "hostId", "workspaceId", "credential", "protocolVersion", "cursor",
    "lastAttemptAt", "lastSuccessAt", "lastErrorCode",
  ], ["projectionRevisionOffset"], "Cloud connection");
  const projectionRevisionOffset = connection.projectionRevisionOffset ?? 0;
  if (
    ![1, 2].includes(connection.protocolVersion) || !Number.isSafeInteger(connection.cursor) || connection.cursor < 0 ||
    !Number.isSafeInteger(projectionRevisionOffset) || projectionRevisionOffset < 0
  ) {
