...
const OUTCOME_REASONS = Object.freeze({
  applied: new Set(["WORK_ENQUEUED", "WORK_REMOVED"]),
  rejected: new Set([
    "STALE_ITEM_REVISION",
    "WORK_ITEM_EXISTS",
    "WORK_ITEM_NOT_FOUND",
...
export function validateCommandReceipt(value) {
  ...
  if (typeof input.reasonCode !== "string" || !OUTCOME_REASONS[input.outcome]?.has(input.reasonCode)) {
  ...
  for (const candidate of events) {
    ...
    if (event.type === "command.requested") {
      ...
      if (
        ...
        items.has(event.payload.workItem.workItemId)
      ) {
      ...
      if (
        ...
        (receipt.outcome === "applied") !== facts.has(intent.intentId) ||
        receipt.workItemId !== (intent.type === "enqueue_work"
